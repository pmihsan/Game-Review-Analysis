...
public class Sentiment_Analysis extends Configured implements Tool {
	public static class Map extends Mapper<LongWritable, Text, Text, Text>{
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException{
			...
			try{
				System.out.println("Tuple: " + tuple.length);
				for(int i=0;i<tuple.length; i++){
					// System.out.println(tuple[i]);
					JSONObject obj =(JSONObject) jsonParser.parse(line);
					//System.out.println(obj);
					String id = String.valueOf(obj.get("id"));
					String game_review=String.valueOf(obj.get("review"));
					String game_id=String.valueOf(obj.get("game_id"));
					twt=(String) obj.get("review");
					...
					//context.write(new Text(game_id + " ::: "),new Text(game_review+" ::: "+new Text(Integer.toString(sentiment_sum))));
					context.write(new Text(game_id + ":::" + game_review),new Text(":::" + Integer.toString(sentiment_sum)));
				}
			}catch(Exception e){
				e.printStackTrace();
			}
	...
	// TODO Auto-generated method stub
		...
		DistributedCache.addCacheFile(new URI("/affin/affin.txt"),conf);
